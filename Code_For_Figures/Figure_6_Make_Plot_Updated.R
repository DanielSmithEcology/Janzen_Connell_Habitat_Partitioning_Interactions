...
#   Adds an overlaid JC-only Dispersal-Limited curve (red triangles)
#   on the JC-only richness panel, with a math-styled legend for σ_D.
#
...
f_only_Disp_Lim <- file.path(fig6_base, "Fig_6_JCE_Only_Disp_Lim", "Summary_JCE_Only_Disp_Lim.csv")
...
# Dispersal-limited JC-only series
d_only_disp <- readr::read_csv(f_only_Disp_Lim, show_col_types = FALSE)
...
plot_with_global <- function(df_info, yvar, ylab,
  ...
  if (!is.null(group_var)) {
    ...
    # If labels_map provided as CHARACTER strings like "sigma[D]==infinity", parse to expressions
    if (!is.null(labels_map)) {
      if (is.character(labels_map)) {
        lbl_chr <- unname(labels_map[lvls])
        lbls <- parse(text = lbl_chr)
      } else if (is.list(labels_map)) {
        # list of quoted calls or expressions
        lbls <- as.expression(labels_map[lvls])
      } else if (is.expression(labels_map)) {
        lbls <- labels_map[lvls]
      } else {
        stop("labels_map must be character, list of expressions, or an expression vector.")
      }
    } else {
    ...
    # Legend title: accept character "sigma[D]" and parse to expression safely
    if (!is.null(legend_title)) {
      if (is.character(legend_title)) {
        lt <- parse(text = legend_title)[[1]]
      } else {
        lt <- legend_title  # assume already an expression
      }
...
# ---------- Prep datasets ----------
# JC-only HP panel prep (for fun panel override below)
d_only_info <- prep_global(d_only, global_offset_factor = 0.35)

# HP panel prep (with range_str factor set above)
d_hp_info   <- prep_global(d_hp,   global_offset_factor = 0.35)

# JC-only richness OVERLAY: bind base (σ_D = ∞) and dispersal-limited (σ_D = 1.0)
...
# carry fun_val in case we reuse; harmless for richness
d_only_both <- d_only_both %>%
...
# ---------- Build panels ----------
# (A) JC-only Species richness with overlay and math legend for sigma_D
p_only_rich <- plot_with_global(
  ...
  labels_map     = c(
    "infty" = "sigma[D]==infinity",
    "1_0"   = "sigma[D]==1.0"
  )
)

# move legend a bit down in p_only_rich only
p_only_rich <- p_only_rich +
  theme(
    legend.position      = c(0.98, 0.86),   # x in [0,1] from left; y from bottom
    legend.justification = c("right", "top")
  )



# (B) JC-only functional value panel (no overlay needed, unchanged)
