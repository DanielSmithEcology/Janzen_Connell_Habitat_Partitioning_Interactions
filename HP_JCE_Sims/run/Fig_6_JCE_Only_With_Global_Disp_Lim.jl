...
# ========= Output dir =========
output_dir = joinpath(@__DIR__, "..", "Outputs", "Fig_6", "simulation_outputs_Fig_6", "Fig_6_JCE_Only_Disp_Lim")
...
const DISP_ON       = true
const DISP_LIM      = 6
const DISP_SCALE    = 1.0           # required but unused if dispersal off
const TEMP_RHO      = 0.0
...
for K in K_vals, λ in λ_vals, range_val in range_vals
    ...
    push!(tasks, @spawn begin
        ...
        CSV.write(joinpath(output_dir, "comm_GLOBAL_K$(K)_range_$(range_str)_lambda_$(λ_str)_Disp_Lim.csv"), DataFrame(comm, :auto))
        CSV.write(joinpath(output_dir, "props_GLOBAL_K$(K)_range_$(range_str)_lambda_$(λ_str)_Disp_Lim.csv"), DataFrame(props, :auto))
...
CSV.write(joinpath(output_dir, "Summary_JCE_Only_Disp_Lim.csv"), df)
println("Saved: ", joinpath(output_dir, "Summary_JCE_Only_Disp_Lim.csv"))
...
#include("run/Fig_6_JCE_Only_With_Global_Disp_Lim.jl")