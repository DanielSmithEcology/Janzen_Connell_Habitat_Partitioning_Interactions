...
output_dir = joinpath(@__DIR__, "..", "Outputs", "Appendix_SST", "High_Auto_Disp_Lim")
...
# === Dispersal settings (now ON, and used in file naming as sigmaD_XX) ===
dispersal_on    = true
dispersal_scale = 1.5     # this will appear in filenames as sigmaD_15
sigmaD_str      = replace(string(round(dispersal_scale, digits=4)), "." => "")  # 1.5 -> "15"
...
for neigh in Moore_Neigh_vals, Z in Z_vals
    push!(tasks, @spawn begin
        ...
        config = SimulationConfig(
            grid_size = 175,
            n_species = 500,
            n_generations = 50000,     # generations ≈ n_generations * mortality_rate
            sigma = 0.05,               # unused under :tolerance_tradeoff; kept for compatibility
            JC_strength = JanCon_a,
            dispersal_on = dispersal_on,    # <<< TURNED ON
            dispersal_limit = 6,
            mortality_rate = 0.2,
            immigration_rate = 0.3e-5,
            dispersal_scale = dispersal_scale,  # 1.5
        ...
        # === Save main outputs (match sigmaD naming like the second script) ===
        # Example: ..._sigmaD_15.csv   for dispersal_scale = 1.5
        CSV.write(joinpath(output_dir,
            "comm_neigh_$(neigh_str)_Z_$(Z_str)_lambda_$(lambda_str)_Disp_Lim_sigmaD_$(sigmaD_str).csv"),
            DataFrame(comm, :auto))

        CSV.write(joinpath(output_dir,
            "props_neigh_$(neigh_str)_Z_$(Z_str)_lambda_$(lambda_str)_Disp_Lim_sigmaD_$(sigmaD_str).csv"),
...
# === Save final richness summary (include sigmaD in the filename as well) ===
df_richness = DataFrame(richness_results)
CSV.write(joinpath(output_dir,    "Richness_M_Z_Disp_Lim_sigmaD_$(sigmaD_str)_High_Autocorr.csv"), df_richness)
...
#include("run/Appendix_SST_High_AutoCor_SigmaD_15.jl")
