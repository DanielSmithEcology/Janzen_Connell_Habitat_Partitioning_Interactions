...
# === Output directory ===
output_dir = joinpath(@__DIR__, "..", "Outputs", "Fig_3", "simulation_outputs_Fig_3D")
...
for neigh in Moore_Neigh_vals, σ in sigma_vals
    push!(tasks, @spawn begin
        ...
        # === Save main outputs ===
        CSV.write(joinpath(output_dir, "comm_neigh_$(neigh_str)_sigma_$(sigma_str)_lambda_$(lambda_str)_sigmaD_15.csv"), DataFrame(comm, :auto))
        CSV.write(joinpath(output_dir, "props_neigh_$(neigh_str)_sigma_$(sigma_str)_lambda_$(lambda_str)_sigmaD_15_.csv"), DataFrame(props, :auto))
...
CSV.write(joinpath(output_dir, "Richness_M_sigmah_Disp_Lim_sigmaD_1_5_High_Autocorr.csv"), df_richness)
...
#include("run/Fig_3D_Moore_Sigma_High_AutoCor_Disp_Lim_SigmaD_15.jl")
